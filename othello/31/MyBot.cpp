...
Move MyBot::play( const OthelloBoard& board )
{
    ...
    else
    {
       ...
       Res r=minimax(turn,0,board,turn);
       Move chiru=r.mv; 
       return Move(chiru.x,chiru.y);
    }
...
Res MyBot::minimax(Turn turn,int depth,const OthelloBoard& board,Turn turn1)
{
    ...
    else 
    {
        ...
        if( moves.size()!=0)
        {
         ...
         {
            OthelloBoard tempB=OthelloBoard(board);
            tempB.makeMove(turn,*it);
            j++;
            int temp=minimax(other(turn),depth+1,tempB,turn1).value;
            if(j==1)
            {
            	r.mv=*it;
            	r.value=temp;
        ...
        else
        {
         ...
         {
            OthelloBoard tempB=OthelloBoard(board);
            tempB.makeMove(other(turn),*it);
            j++;
            int temp=minimax(turn,depth,tempB,turn1).value;
            if(j==1)
            {
            	r.mv=*it;
            	r.value=temp;
